...
def click_verify(driver: ChromiumPage) -> DataPacket:
    try:
        cf_turnstile = driver.ele("@name=cf-turnstile-response", timeout=6)
        cf_entry = cf_turnstile.parent()
        cf_iframe = cf_entry.shadow_root.ele("tag:iframe", timeout=6)
        cf_body = cf_iframe.ele("tag:body", timeout=5).shadow_root
        cf_button = cf_body.ele("tag:input", timeout=5)
        cf_button.focus()
